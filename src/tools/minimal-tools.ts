...
export function registerMinimalTools(mcp: FastMCP): void {
  ...
  mcp.addTool({
    ...
    parameters: z.object({
      force: z.boolean().default(false).describe('Force re-authentication even if already connected')
    }),
    execute: async (_args) => {
      try {
        const result = await startOAuthFlow();
        
        let responseText = `# QuickBooks Authentication\n\n` +
        ...
        if (result.refreshToken) {
          responseText += `## 🔑 Authentication Tokens\n\n` +
        ...
        responseText += `### Available Resources\n` +
  ...
  mcp.addTool({
    ...
    parameters: z.object({
      confirm: z.boolean().default(false).describe('Confirm that you want to clear authentication')
    }),
    execute: async (args) => {
      if (!args.confirm) {
        return {
      ...
      try {
        ...
        throw new Error(`Failed to clear authentication: ${error instanceof Error ? error.message : error}`);
      }
    }
  });
}